...
- [x] 1. Scaffold project structure and static assets
...
- [x] 2. Create static JSON data files
  - [x] 2.1 Create `data/matches.json` with at least 6 match objects (mix of upcoming and past dates) conforming to the Match schema: `id`, `homeTeam`, `awayTeam`, `date` (YYYY-MM-DD), `time` (HH:MM), `venue`, `hasTicket`, `totalCapacity`, `reservedSeats`
    - _Requirements: 2.1, 2.3, 4.1, 4.8_

  - [x] 2.2 Create `data/ticketCategories.json` with exactly three objects: `id` ∈ `{normal, vip, kids}`, `name`, `price`, `description` (≤ 150 chars), `benefits`; enforce `normal.price ≤ vip.price` and `normal.price ≤ kids.price`; VIP must list ≥1 exclusive benefit; Kids description must reference "under 12"
    - _Requirements: 3.1, 3.3, 3.4, 3.5_

  - [x] 2.3 Create `data/players.json` with at least 11 player objects conforming to Player schema: `id`, `name`, `jerseyNumber` (1–99), `position` ∈ `{Goalkeeper, Defender, Midfielder, Forward}`, `photoUrl`; include at least one player with empty `photoUrl`
    - _Requirements: 5.1, 5.4, 5.5_

  - [x] 2.4 Create `data/coaches.json` with at least 3 coach objects conforming to Coach schema: `id`, `name`, `role`, `photoUrl`; include at least one coach with empty `photoUrl`
    - _Requirements: 5.2, 5.4_

- [x] 3. Implement `js/data.js` — async fetch helpers
  - [x] 3.1 Implement `loadMatches()`, `loadPlayers()`, `loadCoaches()`, and `loadTicketCategories()` as async functions returning `{ ok: true, data }` on success and `{ ok: false, error }` on network or parse failure; wrap every `fetch()` in try/catch; check `res.ok` before parsing JSON
    - _Requirements: 2.6_
